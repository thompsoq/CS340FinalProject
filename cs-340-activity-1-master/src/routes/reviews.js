...
router.post('/reviews/search', (req, res, next) => {
    let context = createViewContext();
	req.db.query('SELECT * FROM Series s, Episodes e WHERE s.title = ? AND e.ep_num = ?', [req.body.title, req.body.ep_num], (err, results) => {	
		if (err) return next(err);
        if (results.length) {
			req.db.query(
				`
			...
			context.message = `Can't search because either series title ${req.body.title} or episode number ${req.body.ep_num} doesn't exist`;
			res.render('reviews-search', context);	
		}
	});
});
